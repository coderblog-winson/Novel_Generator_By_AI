...
def create_label_with_help(self, parent, label_text, tooltip_key, row, column,
    ...
    """
    Encapsulate a Label with "?" button to display prompt information.
    """
...
def build_config_tabview(self):
    """
    Create a tab that contains LLM Model settings and Embedding settings.
    """
    ...
    # The "Save Configuration" and "Load Configuration" buttons at the bottom
    ...
    save_config_btn = ctk.CTkButton(self.btn_frame_config, text=_("Save current configuration"), command=self.save_config_btn, font=("Microsoft YaHei", 12))
    save_config_btn.grid(row=0, column=0, padx=5, pady=5, sticky="ew")

    load_config_btn = ctk.CTkButton(self.btn_frame_config, text=_("Load current configuration"), command=self.load_config_btn, font=("Microsoft YaHei", 12))
...
def build_ai_config_tab(self):
    ...
    # 1) API Key
    create_label_with_help(self, parent=self.ai_config_tab, label_text=_("LLM API Key:"), tooltip_key="api_key", row=0, column=0, font=("Microsoft YaHei", 12))
    ...
    # 2) Base URL
    create_label_with_help(self, parent=self.ai_config_tab, label_text=_("LLM Base URL:"), tooltip_key="base_url", row=1, column=0, font=("Microsoft YaHei", 12))
    ...
    # 3) Interface format
    create_label_with_help(self, parent=self.ai_config_tab, label_text=_("LLM Interface format:"), tooltip_key="interface_format", row=2, column=0, font=("Microsoft YaHei", 12))
    ...
    # 4) Model Name
    create_label_with_help(self, parent=self.ai_config_tab, label_text=_("Model Name:"), tooltip_key="model_name", row=3, column=0, font=("Microsoft YaHei", 12))
    ...
    # 5) Temperature
    create_label_with_help(self, parent=self.ai_config_tab, label_text=_("Temperature:"), tooltip_key="temperature", row=4, column=0, font=("Microsoft YaHei", 12))
    ...
    # 6) Max Tokens
    create_label_with_help(self, parent=self.ai_config_tab, label_text=_("Max Tokens:"), tooltip_key="max_tokens", row=5, column=0, font=("Microsoft YaHei", 12))
    ...
    # 7) Timeout (sec)
    create_label_with_help(self, parent=self.ai_config_tab, label_text=_("Timeout (sec):"), tooltip_key="timeout", row=6, column=0, font=("Microsoft YaHei", 12))
    ...
    # Add a test button
    test_btn = ctk.CTkButton(self.ai_config_tab, text=_("Test configuration"), command=self.test_llm_config, font=("Microsoft YaHei", 12))
...
def build_embeddings_config_tab(self):
    ...
    # 2) Embedding Interface format
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text=_("Embedding Interface format:"), tooltip_key="embedding_interface_format", row=1, column=0, font=("Microsoft YaHei", 12))
    ...
    # Add a test button
    test_btn = ctk.CTkButton(self.embeddings_config_tab, text=_("Test configuration"), command=self.test_embedding_config, font=("Microsoft YaHei", 12))
...
def load_config_btn(self):
    ...
    if cfg:
        ...
        self.genre_var.set(other_params.get("genre", _("Fantasy")))
        ...
        self.log(_("The configuration is loaded."))
    else:
        messagebox.showwarning(_("Info"), _("Configuration file not found or cannot be read."))
...
def save_config_btn(self):
    ...
    if save_config(existing_config, self.config_file):
        messagebox.showinfo(_("Info"), _("Configuration saved to %s") % "config.json")
        self.log(_("The configuration has been saved."))
    else:
        messagebox.showerror(_("Error"), _("Save configuration failed."))
