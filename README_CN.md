...
此项目是基于 [AI_NovelGenerator](https://github.com/YILING0013/AI_NovelGenerator) 做的修改！

## 💎 除了现有的官方功能外，我还额外添加了一些的新功能：
1. 支持生成多种语言的小说，同时界面也支持多语言,目前只有中文和英文,但可以方便地添加其他语言
2. 每次启动时，自动加载当前章节
3. 优化了扩写功能，原版只能扩写一次并且每次是删除了原内容重写，但由于 AI 模型的限制，一次是不能写太长的内容的，因此现在我的做法就是在原有内容上继写，每次将新生的内容叠加上去，这样经过多次操作后，就可以产生足够长的内容
4. 在扩写时也支持了英文计数
5. 为每个内容页面添加了一键按钮，方便直接将当前内容复制出来
6. 可以在编辑器里加大或者减少字体的大小

以下是官方说明（我基于此修改版本做了一些改动）：
...
├── prompt/[lang code].py        # 定义多语言的 AI 提示词
...
4. **点击「Step3. 生成章节草稿」**  
   ...
   - 点击按钮后，系统将：  
     - 自动读取前文设定、`Novel_outline.txt`、以及已定稿章节  
     - 调用向量检索回顾剧情，保证上下文连贯  
